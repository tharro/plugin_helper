...
class MyWidgetFlutterSlider extends StatefulWidget {
  @override
  final Key? key;
  final Axis axis;
  final double? handlerWidth;
  final double? handlerHeight;
  final MyWidgetFlutterSliderHandler? handler;
  final MyWidgetFlutterSliderHandler? rightHandler;
  ...
  final List<MyWidgetFlutterSliderFixedValue>? fixedValues;
  final bool rangeSlider;
  final bool rtl;
  final bool jump;
  final bool selectByTap;
  final List<MyWidgetFlutterSliderIgnoreSteps> ignoreSteps;
  final bool disabled;
  final double? touchSize;
  final bool visibleTouchArea;
  final double minimumDistance;
  final double maximumDistance;
  final MyWidgetFlutterSliderHandlerAnimation handlerAnimation;
  final MyWidgetFlutterSliderTooltip? tooltip;
  final MyWidgetFlutterSliderTrackBar trackBar;
  final MyWidgetFlutterSliderStep step;
  final MyWidgetFlutterSliderHatchMark? hatchMark;
  ...
  MyWidgetFlutterSlider(
      ...
      this.tooltip,
      this.trackBar = const MyWidgetFlutterSliderTrackBar(),
      this.handlerAnimation = const MyWidgetFlutterSliderHandlerAnimation(),
      this.selectByTap = true,
      this.step = const MyWidgetFlutterSliderStep(),
      this.hatchMark,
...
//        assert( fixedValues == null || (fixedValues != null && values.length > 1 && values[1] >= values[0] && values[1] <= 100), "When using fixedValues, you should set values within the range of fixedValues" ),
  ...
  @override
  _MyWidgetFlutterSliderState createState() => _MyWidgetFlutterSliderState();
}

class _MyWidgetFlutterSliderState extends State<MyWidgetFlutterSlider>
  ...
  late MyWidgetFlutterSliderTooltip _tooltipData;
  ...
  double? _widgetMin;
  double? _widgetMax;
  List<MyWidgetFlutterSliderIgnoreSteps> _ignoreSteps = [];
  final List<MyWidgetFlutterSliderFixedValue> _fixedValues = [];
  ...
  @override
  void didUpdateWidget(MyWidgetFlutterSlider oldWidget) {
    __isInitCall = false;
...
//                  ..._points,
  ...
  void initMethod() {
    ...
    switch (_tooltipData.direction) {
      case MyWidgetFlutterSliderTooltipDirection.top:
        animationFinish = const Offset(0, -1);
        break;
      case MyWidgetFlutterSliderTooltipDirection.left:
        animationFinish = const Offset(-1, 0);
        break;
      case MyWidgetFlutterSliderTooltipDirection.right:
        animationFinish = const Offset(1, 0);
        break;
      default:
  ...
  void _drawHatchMark() {
    ...
    MyWidgetFlutterSliderHatchMark hatchMark = MyWidgetFlutterSliderHatchMark();
    ...
    hatchMark.smallLine = widget.hatchMark!.smallLine ??
        const MyWidgetFlutterSliderSizedBox(
            height: 5,
            width: 1,
    ...
    hatchMark.labelBox = widget.hatchMark!.labelBox ??
        const MyWidgetFlutterSliderSizedBox(height: 50, width: 50);
...
//      left = hatchMark.linesDistanceFromTrackBar - 3.62;
      ...
      if (widget.axis == Axis.horizontal) {
        if (hatchMark.linesAlignment ==
            MyWidgetFlutterSliderHatchMarkAlignment.left) {
      ...
      for (int p = 0; p <= percent; p++) {
        MyWidgetFlutterSliderSizedBox? barLineBox = hatchMark.smallLine;
...
//        left = (p * distance) + _handlersPadding - labelBoxHalfSize - 0.5;
          ...
          if (hatchMark.linesAlignment ==
              MyWidgetFlutterSliderHatchMarkAlignment.right) {
    ...
    if (hatchMark.labels != null && hatchMark.labels!.isNotEmpty) {
      ...
      double? tr;
      for (MyWidgetFlutterSliderHatchMarkLabel markLabel in hatchMark.labels!) {
...
//          left = (tr * distance) + _handlersPadding - labelBoxHalfSize - 0.5;
  ...
  void _setParameters() {
    ...
    if (widget.fixedValues != null && widget.fixedValues!.isNotEmpty) {
      ...
      for (MyWidgetFlutterSliderFixedValue fixedValue in widget.fixedValues!) {
      ...
      for (double fixedPercent = 0; fixedPercent <= 100; fixedPercent++) {
        dynamic fValue = '';
        for (MyWidgetFlutterSliderFixedValue fixedValue
          ...
          if (fixedValue.percent == fixedPercent.toInt()) {
            ...
            if (fixedPercent > lowerIgnoreBound + 1 || lowerIgnoreBound == 0) {
              ...
              _ignoreSteps.add(MyWidgetFlutterSliderIgnoreSteps(
        ...
        _fixedValues.add(MyWidgetFlutterSliderFixedValue(
      ...
      if (!fixedV.contains(100)) {
        _ignoreSteps.add(
            MyWidgetFlutterSliderIgnoreSteps(from: biggestPoint! + 1, to: 101));
      }
    } else {
    ...
    MyWidgetFlutterSliderTooltip widgetTooltip =
        widget.tooltip ?? MyWidgetFlutterSliderTooltip();

    _tooltipData = MyWidgetFlutterSliderTooltip();
    _tooltipData.boxStyle = widgetTooltip.boxStyle ??
        MyWidgetFlutterSliderTooltipBox(
    ...
    _tooltipData.direction =
        widgetTooltip.direction ?? MyWidgetFlutterSliderTooltipDirection.top;
  ...
  void _generateHandler() {
    /*Right Handler Data*/
    MyWidgetFlutterSliderHandler inputRightHandler =
        widget.rightHandler ?? MyWidgetFlutterSliderHandler();
  ...
  bool _leftHandlerIgnoreSteps(double? tS) {
    ...
    if (_ignoreSteps.isNotEmpty) {
      ...
      for (MyWidgetFlutterSliderIgnoreSteps steps in _ignoreSteps) {
        if (((!widget.rtl) &&
  ...
  void _checkRangeStep(double realValue) {
    ...
    if (widget.step.rangeList != null) {
      for (MyWidgetFlutterSliderRangeStep rangeStep in widget.step.rangeList!) {
...
//    items      ..addAll(_points);
  ...
  Positioned _tooltip(
    ...
    if (_tooltipData.direction == MyWidgetFlutterSliderTooltipDirection.top) {
...
class _MakeHandler extends StatelessWidget {
  ...
  final double? height;
  final GlobalKey? id;
  final MyWidgetFlutterSliderHandler? handlerData;
  final bool? visibleTouchArea;
  ...
  Widget build(BuildContext context) {
    ...
    MyWidgetFlutterSliderHandler handler =
        handlerData ?? MyWidgetFlutterSliderHandler();
...
class MyWidgetFlutterSliderHandler {
...
class MyWidgetFlutterSliderTooltip {
  Widget Function(dynamic value)? custom;
  String Function(String value)? format;
  TextStyle? textStyle;
  MyWidgetFlutterSliderTooltipBox? boxStyle;
  Widget? leftPrefix;
  Widget? leftSuffix;
  Widget? rightPrefix;
  Widget? rightSuffix;
  ...
  MyWidgetFlutterSliderTooltipDirection? direction;
  MyWidgetFlutterSliderTooltipPositionOffset? positionOffset;

  MyWidgetFlutterSliderTooltip({
    this.custom,
    this.format,
    this.textStyle,
    this.boxStyle,
...
class MyWidgetFlutterSliderTooltipPositionOffset {
  double? top;
  double? left;
  double? right;
  double? bottom;

  MyWidgetFlutterSliderTooltipPositionOffset(
...
class MyWidgetFlutterSliderTooltipBox {
  final BoxDecoration? decoration;
  final BoxDecoration? foregroundDecoration;
  final Matrix4? transform;

  const MyWidgetFlutterSliderTooltipBox(
...
class MyWidgetFlutterSliderTrackBar {
  ...
  const MyWidgetFlutterSliderTrackBar({
...
class MyWidgetFlutterSliderIgnoreSteps {
  final double? from;
  final double? to;

  MyWidgetFlutterSliderIgnoreSteps({this.from, this.to})
...
class MyWidgetFlutterSliderFixedValue {
  final int? percent;
  final dynamic value;

  MyWidgetFlutterSliderFixedValue({this.percent, this.value})
      : assert(
...
class MyWidgetFlutterSliderHandlerAnimation {
  final Curve curve;
  final Curve? reverseCurve;
  final Duration duration;
  final double scale;

  const MyWidgetFlutterSliderHandlerAnimation(
...
class MyWidgetFlutterSliderHatchMark {
  bool disabled;
  double density;
  double? linesDistanceFromTrackBar;
  double? labelsDistanceFromTrackBar;
  List<MyWidgetFlutterSliderHatchMarkLabel>? labels;
  MyWidgetFlutterSliderSizedBox? smallLine;
  MyWidgetFlutterSliderSizedBox? bigLine;
  MyWidgetFlutterSliderSizedBox? labelBox;
  MyWidgetFlutterSliderHatchMarkAlignment linesAlignment;
  bool? displayLines;

  MyWidgetFlutterSliderHatchMark(
      {this.disabled = false,
      this.density = 1,
      ...
      this.smallLine,
      this.bigLine,
      this.linesAlignment = MyWidgetFlutterSliderHatchMarkAlignment.right,
      this.labelBox,
      this.displayLines})
...
class MyWidgetFlutterSliderHatchMarkLabel {
  final double? percent;
  final Widget? label;

  MyWidgetFlutterSliderHatchMarkLabel({
    this.percent,
    this.label,
...
class MyWidgetFlutterSliderSizedBox {
  final BoxDecoration? decoration;
  final BoxDecoration? foregroundDecoration;
  final Matrix4? transform;
  final double width;
...
class MyWidgetFlutterSliderStep {
  final double step;
  final bool isPercentRange;
  final List<MyWidgetFlutterSliderRangeStep>? rangeList;

  const MyWidgetFlutterSliderStep({
    this.step = 1,
...
class MyWidgetFlutterSliderRangeStep {
  final double? from;
  final double? to;
  final double? step;

  MyWidgetFlutterSliderRangeStep({
    this.from,
    this.to,
    this.step,
...
enum MyWidgetFlutterSliderTooltipDirection { top, left, right }

enum MyWidgetFlutterSliderHatchMarkAlignment { left, right }
